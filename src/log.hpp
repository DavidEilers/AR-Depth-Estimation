...
class Logger
{
  ...
  public:
    Logger(std::string prefix_, std::ostream *stream_, bool enabled_)
        : m_prefix{prefix_}, m_stream{stream_}, m_enabled{enabled_}
    {
    }

    template <typename T> Logger operator<<(T out)
    {
        if (m_enabled)
        {
