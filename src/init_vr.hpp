...
class Vr
{
  ...
  public:
    ...
    void update_camera_transform_matrix(){
        ...
        cam_to_eye_mat[0] = (glm::inverse(camera_projection[0]))*camera_to_head_mat[0]*(glm::inverse(eye_to_head_transform[0]))*eye_proj[0];
        cam_to_eye_mat[1] = (glm::inverse(camera_projection[1]))*camera_to_head_mat[1]*(glm::inverse(eye_to_head_transform[1]))*eye_proj[1];
